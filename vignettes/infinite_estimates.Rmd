...
decides if the maximum likelihood estimate (MLE) has infinite components.
...
I recommend to set `maxit = 10000` when estimating LBRMs.
...
`glm` will often require user specified starting values for the LBRM. Valid starting values
have to reside in the interior of the feasible region ($X \beta < 0$).
...
I found no conclusive evidence that one of these initialization methods outperforms
