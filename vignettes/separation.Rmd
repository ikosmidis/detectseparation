...
```{r, echo = TRUE, eval = TRUE}
(inf_check <- check_infinite_estimates(modML))
plot(inf_check)
```
...
```{r, echo = TRUE,  eval = TRUE}
coef(modML) + coef(endometrial_sep)
```
and the estimated standard errors are
```{r, echo = TRUE,  eval = TRUE}
coef(summary(modML))[, "Std. Error"] + abs(coef(endometrial_sep))
```
...
```{r, echo = TRUE, eval = TRUE}
update(endometrial_sep, solver = "glpk")
```
or use tge direct [`lpSolveAPI`](https://CRAN.R-project.org/package=ROI.plugin.glpk) implementation
```{r,  echo = TRUE, eval = TRUE}
update(endometrial_sep, implementation = "lpSolveAPI")
```
